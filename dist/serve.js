...
class MCPServerManager {
    servers = new Map();
    requestIdCounter = 0;
    nextRequestId() { return ++this.requestIdCounter; }
    async startServer(config) {
        return new Promise((resolve, reject) => {
            ...
            child.stderr?.on('data', (data) => {
                ...
                if (output)
                    logger.warn(`[${config.name}] stderr:`, output);
            });
            child.on('error', (err) => {
    ...
    sendNotification(server, notification) {
    ...
    getNextRequestId() { return this.nextRequestId(); }
}
// ============== Router ==============
class MCPRouter {
    config;
    ...
    setupRoutes() {
        ...
        this.app.get('/mcp/:server', (req, res) => {
            ...
            if (!srv) {
                res.status(404).json({ error: `Server '${req.params.server}' not found` });
                return;
            }
    ...
    if (config.https) {
        if (!fs.existsSync(config.https.key) || !fs.existsSync(config.https.cert)) {
            ...
            if (!fs.existsSync(mkcertPath))
                mkcertPath = path.join(projectDir, 'mkcert.exe');
            if (fs.existsSync(mkcertPath)) {
                try {
