...
function getProjectDir() {
    if (process.env.MCP_ROUTER_HOME)
        return process.env.MCP_ROUTER_HOME;
...
function runSync() {
    ...
    if (!existsSync(configPath)) {
        console.error('config.json not found in', PROJECT_DIR);
        process.exit(1);
    }
    const config = JSON.parse(readFileSync(configPath, 'utf-8'));
    const { port, host, managedMcpServers } = config;
    if (!managedMcpServers?.length) {
        console.error('managedMcpServers not configured');
        process.exit(1);
    }
    const protocol = config.https ? 'https' : 'http';
    const mcp3p = managedMcpServers.map((m) => ({
        name: m.name,
    ...
    if (!lib) {
        ...
        const pkgs = readdirSync(join(lad, 'Packages')).filter((d) => d.startsWith('Claude_'));
        if (!pkgs.length)
            throw new Error('Claude package not found');
    ...
    console.log(`Current: ${meta.entries?.find((e) => e.id === aid)?.name || aid}`);
    ...
    for (const m of mcp3p) {
        const tools = Object.entries(m.toolPolicy);
        ...
        if (allow.length)
            console.log(`    allow:   ${allow.map(([t]) => t).join(', ')}`);
        if (ask.length)
            console.log(`    ask:     ${ask.map(([t]) => t).join(', ')}`);
        if (block.length)
            console.log(`    blocked: ${block.map(([t]) => t).join(', ')}`);
    }
    ...
    try {
        ...
        if (existsSync(configPath)) {
            ...
            for (const m of cfg.managedMcpServers || []) {
                policies[m.name] = Object.values(m.toolPolicy || {}).filter((s) => s === 'allow').length;
            }
        }
...
function runStop() {
    ...
    // 回退：杀所有 node
    try {
        execSync('cmd /c "taskkill /F /IM node.exe"', { stdio: 'pipe' });
        console.log('  Stopped (all node processes)');
    }
    catch {
        console.log('  Could not stop server. Run as Administrator if needed.');
    }
}
function startInBackground() {
    ...
    const spin = setInterval(() => {
        if (!ready)
            process.stdout.write('\r  Waiting for MCPs to initialize ' + spinner[tick++ % 4]);
    }, 80);
...
switch (cmd) {
    ...
    case 'cert':
        ...
        break;
    case 'autostart':
        runAutostart(process.argv[3] || 'status');
        break;
    case 'start': {
        try {
            execSync('curl -sk https://localhost:3100/health 2>nul', { stdio: 'pipe' });
